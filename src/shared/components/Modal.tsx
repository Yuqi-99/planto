...
export const Modal = ({ title, msg, buttonTitle, img, className, onClose }: TModal) => {
	// h-full max-h-[90vh] w-full
	return (
		<div className='z-modal fixed flex h-full w-full min-w-[inherit] max-w-[inherit]'>
			<ModalOverlay
				onClose={() => {
					onClose?.();
				}}
			/>
			<div
				className='z-modal-content fixed left-1/2 top-1/2 flex h-auto max-h-[90vh] w-screen min-w-[inherit] max-w-[640px] -translate-x-1/2 -translate-y-1/2 items-center justify-center'
				onClickCapture={() => {
					onClose?.();
				}}
			>
				<div className='z-modal-content absolute flex h-auto max-h-[90vh] w-[94%] flex-col items-center justify-center rounded-xl border border-grey-300 bg-green-700'>
					{/* Modal body */}
